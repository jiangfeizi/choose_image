...
class InputButton(Frame):
    def __init__(self, master):
        Frame.__init__(self, master)

class OutputButton(Frame):
    def __init__(self, master):
        Frame.__init__(self, master)
...
class Core(Frame):
    ...
    def init(self, input_path, output_path, class_list, images_dict):
        ...
        self.listbox.bind('<Double-Button-1>', lambda event: self.post_right_menu(event))
...
class Gui(Tk):
    def __init__(self):
        ...
        menubar.add_command(label='标注', command=self.label_gui)
        menubar.add_command(label='训练', command=self.train_gui)
